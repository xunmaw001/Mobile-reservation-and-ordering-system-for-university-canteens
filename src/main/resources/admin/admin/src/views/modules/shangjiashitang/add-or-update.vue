<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="食堂名称" prop="shitangmingcheng">
          <el-input v-model="ruleForm.shitangmingcheng" 
              placeholder="食堂名称" clearable  :readonly="ro.shitangmingcheng"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="食堂名称" prop="shitangmingcheng">
              <el-input v-model="ruleForm.shitangmingcheng" 
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.shitangtupian" label="食堂图片" prop="shitangtupian">
          <file-upload
          tip="点击上传食堂图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.shitangtupian?ruleForm.shitangtupian:''"
          @change="shitangtupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.shitangtupian" label="食堂图片" prop="shitangtupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.shitangtupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  methods: {
	...
	if(this.ruleForm.shitangtupian!=null) {
		this.ruleForm.shitangtupian = this.ruleForm.shitangtupian.replace(new RegExp(this.$base.url,"g"),"");
	}
...
var objcross = this.$storage.getObj('crossObj');
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shangjiashitang/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shangjiashitangCrossAddOrUpdateFlag = false;
						 this.parent.search();
    ...
    shitangtupianUploadChange(fileUrls) {
	this.ruleForm.shitangtupian = fileUrls;
	this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
